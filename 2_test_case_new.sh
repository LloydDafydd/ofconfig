...
# Launch parallel solver (prefer mpirun; mpirun first then srun fallback)
NTASKS=${SLURM_NTASKS:-24}
LAUNCH_RC=0
echo "Launching with mpirun on $NTASKS ranks (preferred)..."
mpirun -np $NTASKS foamRun -solver incompressibleFluid -parallel > log.foamRun.mpirun 2>&1 || LAUNCH_RC=$?
if [ $LAUNCH_RC -ne 0 ]; then
    echo "mpirun launch failed (code $LAUNCH_RC); attempting srun fallback"
    if command -v srun >/dev/null 2>&1; then
        srun --ntasks=$NTASKS foamRun -solver incompressibleFluid -parallel > log.foamRun.srun 2>&1 || LAUNCH_RC=$?
    else
        echo "srun not available; mpirun failed and no fallback present"
    fi
fi
