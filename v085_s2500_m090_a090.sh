...
#SBATCH --job-name=v085_s2500_m090_a090
...
#SBATCH --output=logs/v085_s2500_m090_a090_%j.out
#SBATCH --error=logs/v085_s2500_m090_a090_%j.err
...
echo "  Velocity: 85 mph (38.00 m/s)"
echo "  Spin rate: 2500 RPM"
echo "  Seam orientation: 90°"
echo "  Spin angle: 90°"
echo "  Expected: See postProcessing results"
echo "Started: $(date)"
echo "=========================================="
...
WORK_DIR="results/v085_s2500_m090_a090"
mkdir -p $WORK_DIR
cd $WORK_DIR
...
VELOCITY=38.00      # 85 mph in m/s
SPIN_RATE=2500     # RPM
SEAM_ANGLE=90       # degrees
SPIN_AXIS_ANGLE=90  # degrees (pure backspin/topspin)
...
#!/bin/bash
# auto-generated post-run consolidation template
# This file is read verbatim by generate_parameter_study.jl and inserted into
# each generated job script after simple token replacement for v085_s2500_m090_a090,
# 85, 2500, 90 and 90.

# --- auto-generated post-run consolidation (appended by generate_parameter_study.jl) ---
echo "Post-run consolidation: searching for forces output..."
MASTER_RESULTS_DIR="$HOME/Isambaseball/results_master"
mkdir -p "$MASTER_RESULTS_DIR"

# Find newest forces file under postProcessing/forces; prefer time-dir locations
FORCE_SRC=$(find postProcessing/forces -type f \( -name 'forces.dat' -o -name 'force.dat' -o -name 'forceCoeffs.dat' \) -print0 2>/dev/null | xargs -0 ls -1t 2>/dev/null | head -n1 || true)
if [ -z "$FORCE_SRC" ]; then
    # fallback to common location used by some postProcess runs
    if [ -f "postProcessing/forces/0/forces.dat" ]; then
        FORCE_SRC="postProcessing/forces/0/forces.dat"
    elif [ -f "postProcessing/forces/0/force.dat" ]; then
        FORCE_SRC="postProcessing/forces/0/force.dat"
    elif [ -f "postProcessing/forces/0/forceCoeffs.dat" ]; then
        FORCE_SRC="postProcessing/forces/0/forceCoeffs.dat"
    fi
fi

if [ -n "$FORCE_SRC" ] && [ -f "$FORCE_SRC" ]; then
    OUT_NAME="forces_v085_s2500_m090_a090.dat"
    cp -f "$FORCE_SRC" "$MASTER_RESULTS_DIR/$OUT_NAME"
    echo "Copied $FORCE_SRC -> $MASTER_RESULTS_DIR/$OUT_NAME"

    cat > "$MASTER_RESULTS_DIR/$OUT_NAME.meta" << META
{ "job": "v085_s2500_m090_a090", "velocity_mph": 85, "spin_rpm": 2500, "seam_deg": 90, "spin_axis_deg": 90, "copied_at": "$(date -u +%Y-%m-%dT%H:%M:%SZ)" }
META

    # remove the working directory to free space
    cd .. || true
    rm -rf "results/v085_s2500_m090_a090"
    echo "Removed working folder results/v085_s2500_m090_a090"
else
    echo "Warning: No forces output found for job v085_s2500_m090_a090; leaving results folder in place for inspection"
fi
