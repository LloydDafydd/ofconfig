...
first_line=""
# If a directory exists at system/surfaceFeaturesDict (some OpenFOAM runs create a
# directory of fragment files), move it aside so we can create a single dictionary
# file that surfaceFeatures can read.
if [ -d system/surfaceFeaturesDict ]; then
    echo "Found directory system/surfaceFeaturesDict — moving to backup to allow writing a single dict file"
    if ! mv system/surfaceFeaturesDict system/surfaceFeaturesDict.bak_$(date +%s) 2>/dev/null; then
        rm -rf system/surfaceFeaturesDict || true
    fi
fi
